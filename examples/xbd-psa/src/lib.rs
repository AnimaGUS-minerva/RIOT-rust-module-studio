...
pub extern fn start() {
    ...
    println!("🧪 test_no_psa_init ... ");
    println!("{}", if test_no_psa_init() == Err(-16000) { "✅" } else { "❌" });
...
fn test_no_psa_init() -> Result<bool, minerva_mbedtls::mbedtls_error> {
    ...
    // `.verify()` should return an error since PSA is not initialized yet!!
    pk_context::new()
